locals {
  ...
  base_domain = "${format("%s.%s", var.deployment_id, substr(data.google_dns_managed_zone.public_zone.dns_name, 0 , length(data.google_dns_managed_zone.public_zone.dns_name) - 1))}"
}
